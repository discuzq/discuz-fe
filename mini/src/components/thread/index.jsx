...
import {
  updateThreadAssignInfoInLists,
  updatePayThreadInfo,
  getThreadCommentList,
} from '@common/store/thread-list/list-business';
...
class Index extends React.Component {
  ...
  changeHeight = (params) => {
    ...
    // 更新帖子组件高度
    getElementRect(this.threadStyleId)
      .then((res) => {
    ...
    return (
      <View
        className={`${styles.container} ${className} ${showBottomStyle && styles.containerBottom} ${
          platform === 'pc' && styles.containerPC
        }`}
        style={{ minHeight: `${minHeight}px` }}
        id={this.threadStyleId}
      >
        {relativeToViewport ? (
          <>
            <View className={styles.header} onClick={unifyOnClick || this.onClick}>
              ...
              />
              {isShowIcon && (
                <View className={styles.headerIcon} onClick={unifyOnClick || this.onClickHeaderIcon}>
                  <Icon name="CollectOutlinedBig" className={styles.collectIcon}></Icon>
                </View>
              )}
            </View>
